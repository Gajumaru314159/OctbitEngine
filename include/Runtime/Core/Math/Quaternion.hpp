...
namespace ob
{
    ...
    public:
        ...
        //===============================================================
        Quat();                                     // デフォルトコンストラクタ
        Quat(f32 _x, f32 _y, f32 _z, f32 _w);       // コンストラクタ(内部値指定)
        ...
        //  オペレータ
        //===============================================================
        Quat operator *  (const Quat& quat) const;  // 乗算演算子(回転)
        Quat operator *= (const Quat& quat);        // 乗算代入演算子(回転)
        Quat operator *  (f32 scalar) const;        // 乗算演算子(回転)
        Quat operator *= (f32 scalar);              // 乗算代入演算子(回転)
        Quat operator /  (const Quat& quat) const;  // 除算演算子(逆回転)
        Quat operator /= (const Quat& quat);        // 除算代入演算子(逆回転)
        bool operator == (const Quat& rhs)const;    // 等価演算子
        bool operator != (const Quat& rhs)const;    // 否等価演算子

        Vec3 operator *  (const Vec3& v) const;     // ベクトル乗算演算子(回転)
        ...
        void   Invert();                            // 回転を逆回転にする


        //===============================================================
        //  セッター
        //===============================================================
        void Set(f32 x, f32 y, f32 z, f32 w);       // 内部値設定
        ...
        Quat   Inverse() const;                     // 逆回転を表すQuaternionを取得
        Matrix GetMatrix() const;                   // 行列表現を取得
        ...
        //  判定
        //===============================================================
        b32 Equals(const Quat& v, f32 tolerance = Mathf::TOLERANCE)const;   // 等価判定(許容範囲指定)
    ...
    public:
        ...
        static f32 Dot(const Quat& a, const Quat& b);                       // 内積
    ...
    //-----------------------------------------------------------
    inline Quat::Quat(f32 _x, f32 _y, f32 _z, f32 _w) :x(_x), y(_y), z(_z), w(_w) {};
    ...
    //! @brief 等価演算子
    //-----------------------------------------------------------
    inline bool Quat::operator == (const Quat& quat)const
    {
        return
    ...
    //! @brief ロール・ピッチ・ヨー設定
    //-----------------------------------------------------------
    inline void Quat::Set(f32 x_, f32 y_, f32 z_, f32 w_)
    {
        x = x_;
        y = y_;
        z = z_;
    ...
    //! @brief 内積
    //-----------------------------------------------------------
    inline f32 Quat::Dot(const Quat& a, const Quat& b)
    {
    ...
    //-----------------------------------------------------------
    inline b32 Quat::Equals(const Quat& v, f32 tolerance)const
    {
        return
