...
namespace ob {
    ...
    public:
        ...
        //  コンストラクタ / デストラクタ
        //===============================================================
        Vec3()noexcept;                                             // コンストラクタ(ゼロ初期化)
        explicit Vec3(f32 scalar)noexcept;                          // コンストラクタ(すべての要素を同じ値で初期化)
        Vec3(f32 x, f32 y, f32 z)noexcept;                          // コンストラクタ(要素を指定して初期化)


        //===============================================================
        //  オペレータ
        //===============================================================
        bool operator == (const Vec3& v) const noexcept;             // 等価演算子
        bool operator != (const Vec3& v) const noexcept;             // 否等価演算子

        Vec3 operator + (const Vec3& v) const noexcept;              // 加算演算子
        Vec3 operator + (f32 bias) const noexcept;                   // 加算演算子(スカラー)
        Vec3 operator - (const Vec3& v) const noexcept;              // 減算演算子
        Vec3 operator - (f32 bias) const noexcept;                   // 減算演算子(スカラー)
        Vec3 operator * (const Vec3& v) const noexcept;              // 乗算演算子
        Vec3 operator * (f32 scale) const noexcept;                  // 乗算演算子(スカラー)
        friend Vec3 operator * (f32 f, const Vec3& v) noexcept;      // 乗算演算子(前方スカラー)
        Vec3 operator / (const Vec3& v) const noexcept;              // 除算演算子
        Vec3 operator / (f32 scale) const noexcept;                  // 除算演算子(スカラー)
        friend Vec3 operator / (f32 f, const Vec3& v) noexcept;      // 除算演算子(前方スカラー)

        Vec3& operator += (const Vec3& v) noexcept;                  // 加算代入演算子
        Vec3& operator += (f32 bias) noexcept;                       // 加算代入演算子(スカラー)
        Vec3& operator -= (const Vec3& v) noexcept;                  // 減算代入演算子
        Vec3& operator -= (f32 bias) noexcept;                       // 減算代入演算子(スカラー)
        Vec3& operator *= (const Vec3& v) noexcept;                  // 乗算代入演算子
        Vec3& operator *= (f32 scale) noexcept;                      // 乗算代入演算子(スカラー)
        Vec3& operator /= (const Vec3& v) noexcept;                  // 除算代入演算子
        Vec3& operator /= (f32 scale) noexcept;                      // 除算代入演算子(スカラー)

        Vec3 operator - () const noexcept;                           // 単項マイナス演算子


        //===============================================================
        //  セッター
        //===============================================================
        void    Set(f32 x, f32 y, f32 z) noexcept;                   // 要素の設定
        void    Set(const Vec3& v) noexcept;                         // 要素の設定
        ...
        Vec3    GetUnitVec(f32 tolerance = Mathf::TOLERANCE)const;                      // 単位ベクトルの取得
        Vec3    GetUnitVecUnsafe()const;                                                // 単位ベクトルの取得(ゼロチェックなし)
        ...
        void    Translate(f32 x, f32 y, f32 z) noexcept;                                // 移動
        void    Translate(const Vec3& add) noexcept;                                    // 移動
        ...
        void    Scale(f32 scale) noexcept;                                              // 拡大
        void    Scale(f32 sx, f32 sy, f32 sz) noexcept;                                 // 拡大
        void    Scale(const Vec3& scale) noexcept;                                      // 拡大

        //===============================================================
        //  判定
        //===============================================================
        b32     IsUnit(f32 tolerance = Mathf::TOLERANCE)const;                          // 単位ベクトルか判定
        b32     Equals(const Vec3& v, f32 tolerance = Mathf::TOLERANCE)const noexcept;  // 等価判定(許容範囲指定)
    ...
    public:
        ...
        //  スタティック関数
        //===============================================================
        static Vec3 Abs(const Vec3& v) noexcept;                                        // ベクトルの各要素の絶対値からベクトルを作成
        static f32  Dot(const Vec3& a, const Vec3& b) noexcept;                         // 内積
        static Vec3 Cross(const Vec3& a, const Vec3& b) noexcept;                       // 外積
        static f32  Dist(const Vec3& a, const Vec3& b);                                 // 二つのベクトルの距離を計算
        static f32  SqrDist(const Vec3& a, const Vec3& b) noexcept;                     // 二つのベクトルの距離の二乗を計算
        static f32  DistXY(const Vec3& a, const Vec3& b);                               // 二つのベクトルのXY平面上の距離を計算
        static f32  SqrDistXY(const Vec3& a, const Vec3& b) noexcept;                   // 二つのベクトルのXY平面上の距離の二乗を計算
        static f32  DistYZ(const Vec3& a, const Vec3& b);                               // 二つのベクトルのYZ平面上の距離を計算
        static f32  SqrDistYZ(const Vec3& a, const Vec3& b) noexcept;                   // 二つのベクトルのYZ平面上の距離の二乗を計算
        static f32  DistXZ(const Vec3& a, const Vec3& b);                               // 二つのベクトルのXZ平面上の距離を計算
        static f32  SqrDistXZ(const Vec3& a, const Vec3& b) noexcept;                   // 二つのベクトルのXZ平面上の距離の二乗を計算
        static f32  MDist(const Vec3& a, const Vec3& b) noexcept;                       // 二つのベクトルのマンハッタン距離を計算
        static Vec3 Lerp(const Vec3& a, const Vec3& b, f32 t) noexcept;                 // 二つのベクトルの線形補間
        static Vec3 Slerp(const Vec3& a, const Vec3& b, f32 t);                         // 二つのベクトルの球面補間
        static Vec3 Max(const Vec3& a, const Vec3& b) noexcept;                         // 二つのベクトルの各要素の大きいほうからベクトルを生成
        static Vec3 Min(const Vec3& a, const Vec3& b) noexcept;                         // 二つのベクトルの各要素の小さいほうからベクトルを生成
        ...
        static Vec3 Mirror(const Vec3& inDir, const Vec3& normal) noexcept;             // 反射ベクトルの計算
        static Vec3 Projection(const Vec3& inDir, const Vec3& normal) noexcept;         // 投影ベクトルの計算
    ...
    //-----------------------------------------------------------------------------
    inline Vec3::Vec3(f32 scalar) noexcept {
    ...
    //! @brief 要素の最大を取得
    //-----------------------------------------------------------------------------
    inline f32 Vec3::GetMaxComponent() const noexcept {
    ...
    //! @brief 要素の絶対値の最大を取得
    //-----------------------------------------------------------------------------
    inline f32 Vec3::GetMaxAbsComponent() const noexcept {
    ...
    //! @brief 要素の最小を取得
    //-----------------------------------------------------------------------------
    inline f32 Vec3::GetMinComponent() const noexcept {
    ...
    //! @brief 要素の絶対値の最小を取得
    //-----------------------------------------------------------------------------
    inline f32 Vec3::GetMinAbsComponent() const noexcept {
    ...
    //! @brief ベクトルの長さの二乗を取得
    //-----------------------------------------------------------------------------
    inline f32 Vec3::GetSqrMag() const noexcept {
        return x * x + y * y + z * z;
    }
    ...
    //-----------------------------------------------------------------------------
    inline Vec3 Vec3::GetUnitVec(f32 tolerance)const {
        Vec3 unitVec = *this;
    ...
    //! @brief      拡大
    //-----------------------------------------------------------------------------
    inline void Vec3::Scale(f32 scale) noexcept {
    ...
    //! @brief      拡大
    //-----------------------------------------------------------------------------
    inline void Vec3::Scale(const Vec3& scale) noexcept {
