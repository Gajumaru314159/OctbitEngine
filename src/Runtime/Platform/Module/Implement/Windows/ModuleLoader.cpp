...
namespace ob {
    namespace platform {
        ...
        ModuleLoader::ModuleLoader(const String& fileName) {
            ...
            if (dll == nullptr) {
                LOG_ERROR_EX(TEXT("System"), TEXT("{0}が見つかりませんでした。"), fileName.c_str());
                return;
            }

            FARPROC proc = GetProcAddress(dll, "GetModule");
            if (proc == nullptr) {
                LOG_ERROR_EX(TEXT("System"), TEXT("{0}にCreateModule()が含まれていません。"), fileName.c_str());
                return;
            }
            ...
            if (!pModule) {
                LOG_ERROR_EX(TEXT("System"), TEXT("モジュールの生成に失敗しました。({0})"), fileName.c_str());
                return;
            }

            m_handle = dll;
            m_moduleLoader = unique_ptr<IModule>(pModule);
        }

        ModuleLoader::~ModuleLoader() {
            m_moduleLoader.release();
