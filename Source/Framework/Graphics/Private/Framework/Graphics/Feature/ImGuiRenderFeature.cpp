...
namespace ob::graphics {
	...
	//! @brief		コンストラクタ
	//@―---------------------------------------------------------------------------
	void ImGuiRenderFeature::render(FG& fg, RenderViewId id, FrameGraphResource target) {
	...
	public:
		...
		void render(FG& fg, FrameGraphResource target) {
			...
			fg.addCallbackPass<ImGuiData>(
				...
				[this, displaySize](const ImGuiData& data, FrameGraphPassResources& resources, void* ctx) {
	...
	private:
		...
		void updateTime()
		{
			...
			if (m_time == DateTime()) {
				...
				io.DeltaTime = abs(TimeSpan(now, m_time).secondsF());
			}
		...
		void updateBuffer() {
			...
			const auto overAllocIndexSize = 10000;
