...
namespace ob::graphics {
		...
		:RenderFeature(scene)
	{
		...
		// scene.registerViewStep<ImGuiStep>();
	}
	...
	//@―---------------------------------------------------------------------------
	void ImGuiRenderFeature::createSteps(RenderStepInjector& injector) {
		injector.add(
			[this](RenderView& view) {
				return new ImGuiStep(
					view,
					[this]() {
						m_notifier.invoke();
					}
	...
	//! @brief		コンストラクタ
	//@―---------------------------------------------------------------------------
	ImGuiStep::ImGuiStep(RenderView& view,Action action) 
		: RenderStep(view)
		, m_draw(action)
	{
		// TODO Window取得方法を変える
		m_window = platform::Window::Main();
		initializeContext();
		initializeResource();
}
	...
	//! @brief		コンストラクタ
	//@―---------------------------------------------------------------------------
	void ImGuiStep::render(FG& fg, FrameGraphResource target) {
		...
		if (m_window) {
			ImGui::GetIO().DisplaySize = { m_window.getSize().x,m_window.getSize().y };
		}
		updateMouse();
		updateKeyboard();
		updateTime();
	...
	//@―---------------------------------------------------------------------------
	void ImGuiStep::initializeContext() {
	...
	//@―---------------------------------------------------------------------------
	void ImGuiStep::initializeResource() {
	...
	void ImGuiStep::updateMouse()
	{
		...
		if (bd == nullptr || !m_window)
			return;

		// ウィンドウフォーカス中の未処理
		const bool focused = m_window.getState().has(WindowState::Focused);
		if (focused)
		{
			// カーソル位置
			auto pos = m_window.getClientPoint(Mouse::GetPos());
