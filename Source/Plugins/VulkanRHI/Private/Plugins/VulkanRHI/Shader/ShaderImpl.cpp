...
namespace ob::rhi::vulkan {
    ...
    ShaderImpl::ShaderImpl(VulkanRHI& rhi, const ShaderCompileDesc& desc)
        : m_name(desc.name)
        , m_stage(desc.stage)
    {
        compile(rhi, desc);
    ...
    ShaderImpl::ShaderImpl(VulkanRHI& rhi,BlobView blob, ShaderStage stage, StringView name)
        : m_name(name)
        , m_stage(stage)
    {
        // チャンクタイプをチェック
    ...
    //! @brief				初期化
    void ShaderImpl::compile(VulkanRHI& rhi, const ShaderCompileDesc& desc) {
